Shader "Joseph&Minions/StylizedSkybox"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Distant Lightning Bolts - Visible jagged bolts on horizon
            float3 generateDistantLightning(float3 worldDir, float time, float3 lightningPosition, float lightningIntensity)
            {
                ...
                // Normalize height within bolt range
                float heightInBolt = (viewHeight - boltBottomHeight) / (boltTopHeight - boltBottomHeight);
                
                // Create jagged bolt pattern using stepped noise
                float2 boltSeed = float2(floor(time * _LightningFrequency * 0.2), 0.789);
                
                // Width multiplier from user control
                float widthMult = _DistantLightningWidth;
                
                // Build cumulative jagged path - each segment has horizontal offset from previous
                float cumulativeOffset = 0.0;
                float boltPattern = 0.0;
                float branchPattern = 0.0;
                
                // 7 segments for more detail
                for(int i = 0; i < 7; i++)
                {
                    float segmentStart = float(i) / 7.0;
                    float segmentEnd = float(i + 1) / 7.0;
                    
                    // Is current height in this segment?
                    float inSegment = step(segmentStart, heightInBolt) * step(heightInBolt, segmentEnd);
                    
                    if(inSegment > 0.5)
                    {
                        // Random offset for this segment
                        float segmentOffset = (hash(boltSeed + float2(i * 0.1, 0.0)) - 0.5) * 0.12;
                        cumulativeOffset += segmentOffset;
                        
                        // Distance from bolt path at this height
                        float distFromBolt = abs(angleDiff - cumulativeOffset);
                        
                        // Main bolt line - thin and bright
                        float bolt = smoothstep(0.008 * widthMult, 0.0, distFromBolt);
                        boltPattern = max(boltPattern, bolt);
                        
                        // Add branch for this segment (50% chance)
                        float branchChance = hash(boltSeed + float2(i * 0.5, 0.9));
                        if(branchChance > 0.5)
                        {
                            // Branch direction (left or right)
                            float branchDir = (hash(boltSeed + float2(i * 0.7, 0.3)) > 0.5) ? 1.0 : -1.0;
                            float branchLength = hash(boltSeed + float2(i * 0.9, 0.6)) * 0.06 + 0.03;
                            float branchOffset = cumulativeOffset + branchDir * branchLength;
                            
                            // Branch is thinner than main bolt
                            float distFromBranch = abs(angleDiff - branchOffset);
                            float branch = smoothstep(0.006 * widthMult, 0.0, distFromBranch) * 0.7;
                            branchPattern = max(branchPattern, branch);
                        }
                    }
                    
                    // Use offset from previous segments for smooth path
                    if(heightInBolt < segmentStart)
                    {
                        float segmentOffset = (hash(boltSeed + float2(i * 0.1, 0.0)) - 0.5) * 0.12;
                        cumulativeOffset += segmentOffset;
                    }
                ...
                // Combine main bolt and branches
                float combinedBolt = max(boltPattern, branchPattern);
                
                // Glow around entire bolt structure - strength controlled by user
                float distFromCenter = abs(angleDiff);
                float glow = smoothstep(0.15, 0.02, distFromCenter) * _DistantLightningGlow * 0.4;
                
                // Combine bolt and glow
                float lightningShape = saturate(combinedBolt + glow);
